...
while IFS= read -r dir; do
    ...
    if [ -d "$expanded_dir/.git" ]; then
        ...
        sync_output=$(git -C "$expanded_dir" ls-files -z | rsync -avi --delete --files-from=- --from0 --human-readable "$expanded_dir" "$dest_subdir" | grep '^>')
        ...
        sync_output=$(rsync -avi --delete --human-readable "$expanded_dir/" "$dest_subdir" | grep '^>')
